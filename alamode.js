...
var alamode = {
  ...
  customChartColors: function (o) {
    ...
    function prepColors(id, colorList) {
      var chart = $("#" + id),
        series =
          chart.find(".nv-bar").length > 0
            ? chart.find(".nv-group")
            : chart.find(".nv-line").length > 0 ||
              chart.find(".nv-areaWrap").length > 0
            ? chart.find(".nv-noninteractive")
            : chart.find(".nv-pie .nv-slice"),
    ...
    function drawColors(id, colorList) {
      ...
      for (var i in colors) {
        chart
          .find(".nv-linesWrap .nv-groups .nv-series-" + m[i])
          .css({ fill: colors[i], stroke: colors[i] });
        chart
          .find(".nv-barsWrap .nv-groups .nv-series-" + m[i] + " rect")
          .css({ fill: colors[i], stroke: colors[i] });

        chart
          .find(".nv-linePlusBar .nv-barsWrap .nv-bars rect")
          .each(function (index) {
            $(this).css({ fill: colors[0], stroke: colors[0] });
          });
        chart
          .find(".nv-linePlusBar .nv-linesWrap .nv-groups .nv-series-0")
          .css({ fill: colors[1], stroke: colors[1] });

        chart
          .find(".nv-scatterWrap .nv-groups .nv-series-" + m[i])
          .css({ fill: colors[i], stroke: colors[i] });
        chart
          .find(".nv-areaWrap .nv-area-" + m[i])
          .css({ fill: colors[i], stroke: colors[i] });
      ...
      for (var i in lineDashes) {
        chart
          .find(".nv-linesWrap .nv-groups .nv-series-" + m[i])
          .css({ "stroke-dasharray": lineDashes[i] });
      }

      chart
        .find(".nv-legendWrap .nv-series .nv-legend-symbol")
        .each(function (i) {
          $(this).css({ fill: colors[i], stroke: colors[i] });
        });
    }
    ...
    function onMouseMove(id, colorList) {
      ...
      $(chart).mousemove(function () {
        ...
        $("html")
          .find(".nvtooltip table .legend-color-guide")
          .each(function (i) {
            if (legend.length == 0 && isBarLength == 0) {
              ...
              var lineColor = chart
                ...
                barColor = chart
                  ...
                  .css("fill");
              if (
                $(this).closest(".nvtooltip").find(".key")[0].innerText == ""
              ) {
              ...
              if (seriesCount == 1) {
                $(this)
                  .find("div")
                  .css({ "background-color": colors[r[seriesCount - i - 1]] });
              }
            } else if (isAreaLength > 0) {
              $(this)
                .find("div")
                .css({ "background-color": colors[isAreaLength - i - 1] });
            } else {
        ...
        $("html")
          .find(".nvtooltip table .legend-color-guide div")
          .css("background-color", sliceColor);
      });
  ...
  addTotalsRow: function (o) {
    ...
    function getColumns(selectedColumns) {
      numberColumns = _.map(
        _.filter(resultColumns, function (c) {
  ...
  retentionHeatmap: function (o) {
    ...
    if (gradientBy === "cohort_column") {
      ...
      cohorts.forEach(function (cohort) {
        ...
        colorsByCohort[cohort] = d3.scale
          .quantize()
          .domain(
            d3.extent(dataByCohort, function (d) {
              return d[gradientColumn];
            })
          )
          .range(colors);
      });
      ...
      pivots.forEach(function (pivot) {
        ...
        colorsByPivot[pivot] = d3.scale
          .quantize()
          .domain(
            d3.extent(dataByPivot, function (d) {
              return d[gradientColumn];
            })
          )
          .range(colors);
      ...
      // table or as a default fallback
      var color = d3.scale
        .quantize()
        .domain(
    ...
    var table = d3
      .select(uniqContainerClass)
      .append("table")
      .attr("class", "mode-retention-heatmap-table");

    table
      .selectAll(".mode-retention-heatmap-table-header")
      .data([0])
      .enter()
      .append("tr")
    ...
    table
      .selectAll(".mode-retention-heatmap-table-row")
      .data(cohorts)
      .enter()
      .append("tr")
  ...
  tableWithBars: function (o) {
    ...
    tr.select("div.positive")
      .style("width", function (d) {
        return `${(d.verified * 100) / d.new_respondents}%`;
      })
      .transition()
      .duration(500)
      .style("width", function (d) {
  ...
  simpleHeatmap: function (o) {
    ...
    table
      .selectAll(".mode-simple-heatmap-table-header")
      .data([0])
      .enter()
      .append("tr")
      ...
      .attr("class", function (d) {
        if (isNaN(d)) {
          return "mode-simple-heatmap-table-header-cell heatmap-string";
        } else {
          return "mode-simple-heatmap-table-header-cell heatmap-number";
        }
      })
      .text(function (d) {
        return d;
    ...
    table
      .selectAll(".mode-simple-heatmap-table-row")
      .data(xVals)
      .enter()
      .append("tr")
      ...
      .style("background", function (d) {
        if (checkShade(d)) {
          return color(Math.max(minValue, Math.min(maxValue, d.value)));
        }
      })
      .attr("class", function (d) {
        return cellClass(d);
  ...
  leafletMap: function (o) {
    ...
    var latColumn = o["lat_column"],
      ...
      centerLng = o["center_lng"] || -98.35,
      zoom = o["starting_zoom"] || 4,
      dotRadius = o["dot_size"] || 0.4,
      dotOpacity = o["dot_opacity"] || 0.8,
    ...
    data.forEach(function (d) {
      if (
        ...
        typeof d[lngColumn] === "number"
      ) {
        validData.push(d);
      }
    });

    var uniqContainerClass = alamode.addContainerElement(
      htmlElement,
      applyFilter
    );
    ...
    var mapHeight =
        height - $(uniqContainerClass + ".mode-graphic-title").height(),
  ...
  chartAnnotations: function (o) {
    ...
    if (groupBy) {
      dataGroups = _.groupBy(data, function (d) {
        return d[groupBy];
      });
    }
    ...
    function drawComments() {
      comments.forEach(function (c, i) {
        ...
        if (pointNumber != -1 && orientation == "v") {
          var pointTranlate = $(chart)
              .find(".nv-point.nv-point-" + pointNumber)
              .attr("transform"),
  ...
  sunburstChart: function (o) {
    ...
    events.forEach(function (e, i) {
      if (e != null) {
        colors[e] = colorRange[i % colorRange.length];
      }
    });
  ...
  zipcodeChoropleth: function (o) {
    ...
    queue()
      .defer(
        d3.json,
        "https://s3-us-west-2.amazonaws.com/mode-alamode/zips_us_topo.json"
      )
      .await(ready);
  ...
  countyChoropleth: function (o) {
    ...
    queue()
      .defer(
        d3.json,
        "https://s3-us-west-2.amazonaws.com/mode-alamode/counties.json"
      )
      .await(ready);
  ...
  stateChoropleth: function (o) {
    ...
    queue()
      .defer(
        d3.json,
        "https://s3-us-west-2.amazonaws.com/mode-alamode/states.json"
      )
      .await(ready);
    ...
    function ready(error, us) {
      d3.select("#mode-state-chorolpleth-" + id)
        ...
        .attr("fill", function (d) {
          return quantize(rateById.get(d.properties[code]));
        })
        .attr("d", path);
    }
  },
  ...
  worldChoropleth: function (o) {
    ...
    queue()
      .defer(
        d3.json,
        "https://s3-us-west-2.amazonaws.com/mode-alamode/world.json"
      )
      .await(ready);
    ...
    function ready(error, world) {
      d3.select("#mode-world-chorolpleth-" + id)
        ...
        .on("mouseover", function (d) {
          ...
          d3.select("#mode-world-chorolpleth-legend-" + id).text(
            country + ": " + value
          );
        })
  ...
  bigNumberSparkline: function (o) {
    ...
    if (x) {
      (xMatch = _.filter(cols, { name: x })), (xType = xMatch[0].type);
    } else {
      xType = "string";
    }
    ...
    d3.select(chartId + " .chart-big-number")
      .append("svg")
  ...
  forceDirectedGraph: function (o) {
    ...
    initialLinks.forEach(function (l) {
      var match1 = links.filter(function (d) {
          return d.target == l.source;
        }),
        match2 = match1.filter(function (d) {
          return d.source == l.target;
        });
    ...
    links = links.sort(function (a, b) {
      return b.edge_size - a.edge_size;
    });
    ...
    var link = svg
      .selectAll(".mode-force-directed-graph-link ")
      .data(bilinks)
      .enter()
      .append("path")
    ...
    var node = svg
      .selectAll(".mode-force-directed-graph-node")
      .data(graph.nodes)
      .enter()
      .append("g")
  ...
  networkMatrix: function (o) {
    ...
    d3.select(uniqContainerClass)
      ...
      .html(
        '<p>Order: <select id="mode-network-matrix-order-picker-' +
          id +
          '">' +
    ...
    var row = g
      .selectAll(".mode-network-matrix-row")
      .data(matrix)
      .enter()
      .append("g")
    ...
    var column = g
      .selectAll(".mode-network-matrix-column")
      .data(matrix)
      .enter()
      .append("g")
    ...
    function row(row) {
      var cell = d3
        ...
        .style("fill", function (d) {
          return nodes[d.x].node_group == nodes[d.y].node_group
            ? colors[nodes[d.x].node_group]
            : "#2B2B2B";
        })
        .on("mouseover", function (d) {
          mouseover(d);
    ...
    function mouseover(p) {
      d3.selectAll(".mode-network-matrix-row-text").classed("active", function (
        d,
        i
      ) {
        return i == p.y;
      });
      d3.selectAll(".mode-network-matrix-column-text").classed(
        "active",
        function (d, i) {
          return i == p.x;
        }
    ...
    d3.select("#mode-network-matrix-order-picker-" + id).on(
      "change",
      function () {
        order(this.value);
      }
    ...
    function order(value) {
      ...
      t.selectAll(".mode-network-matrix-column").attr("transform", function (
        d,
        i
      ) {
        return "translate(" + x(i) + ")rotate(-90)";
  ...
  hive: function (o) {
    ...
    svg
      ...
      .on("mouseout", function (d) {
        ...
        d3.selectAll(".mode-hive-link-selected").attr(
          "class",
          "mode-hive-link"
        );
      });
  ...
  conditionalFormattingByColumn: function (o) {
    ...
    function drawGradient(column, color, colorText) {
      ...
      data.forEach(function (d, i) {
        ...
        if (colorText) {
          cell.css("color", selectedColor);
        } else {
          cell.css({ background: selectedColor, color: textColor });
        }
      });
    }
  ...
  addTableOfContents: function (o) {
    ...
    $(".row").each(function () {
      ...
      cols.each(function () {
        var title;
        var chartId =
          $(this).find("mode-chart").attr("id") ||
          $(this).find("mode-table").attr("id") ||
          $(this).find("mode-python").attr("id");
        if (!chartId) {
          return true;
        }
        if (chartId.includes("chart") || chartId.includes("table")) {
          ...
          if ($(element).find("mode-pivot-table").length > 0) {
            title = document
              ...
              .getElementsByClassName("in-place-edit-text")[0].innerText;
          } else {
            title = document
              .getElementById(chartId)
              .getElementsByClassName("chart-title")[0].innerText;
          }
        ...
        var newlink = $(
          "<a class='scroll-link' href=" +
            ...
            chartId +
            ">" +
            (title.includes("Click to add title") ? "Untitled" : title) +
            "</a>"
        );
        nav.append(newlink);
      });
  ...
  xAnnotations: function (o) {
    ...
    setTimeout(function () {
      var highchartContainer = $("#" + chartId).find(
      ...
      var points = data.filter(function (d) {
        if (d) {
          return xValues.indexOf(d.category) >= 0;
        }
      });
